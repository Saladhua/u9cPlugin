...
namespace YY.U9.Cust.LI.UIPlugIn
{
    public class CommonFunction
    {
        ...
        #region Layout重载设置CheckBox控件
        //设置checkBox控件在UI中
        public static void Layout(IContainer container, IControl ctrl, uint x, uint y)
        {
            Layout(container, ctrl, x, y, 1, 1, Unit.Pixel(0), Unit.Pixel(0), true);
        }
        /// <summary>
        ...
        /// <param name="height">控件高</param>
        /// <param name="isAutoSize">是否自适应大小</param>
        public static void Layout(IContainer container, IControl ctrl, uint x, uint y, int xspan, int yspan,
            Unit width, Unit height, bool isAutoSize)
        {
            IGridLayout gl = container.Layout as IGridLayout;
            if (gl == null) return;
