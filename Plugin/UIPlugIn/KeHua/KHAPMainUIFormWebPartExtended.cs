...
namespace YY.U9.Cust.LI.UIPlugIn
{
    ...
    class KHAPMainUIFormWebPartExtended : ExtendedPartBase
    {
        ...
        public override void AfterRender(IPart part, EventArgs args)
        {
            ...
            if (this._part.Model.APBillHead_APBillLines.RecordCount == 0)
            {
                return;
            }
            ...
            {
                string d5 = item["DescFlexField_PrivateDescSeg4"].ToString();
                ...
                if (DocTypeCode == "YF10" || DocTypeCode == "YF11")
                {
                    foreach (var item in this._part.Model.APBillHead_APBillLines.Records)
                    {
                        ...
                        if (IsIncludeTax == "False")
                        {
                            ...
                            // 核币价税合计   APOCMoney.NonTax + APOCMoney.GoodsTax
                            item["APOCMoneyPriceTaxSum"] = TaxSum;

                            item["APFCMoneyPriceTaxSum"] = item["APOCMoneyPriceTaxSum"];
                            ...
                            item["APFCMoney_NonTax"] = item["APOCMoney_NonTax"];
                            ...
                            item["APOCMoney_TotalMoney"] = TaxSum;  //总金额

                            item["APFCMoney_TotalMoney"] = item["APOCMoney_TotalMoney"];  //总金额
                            #endregion
                            item["APOCMoney_GoodsTax"] = decimal.Parse(NonTax) * (decimal.Parse(item["TaxRate"].ToString())); //税额

                            item["APFCMoney_GoodsTax"] = item["APOCMoney_GoodsTax"]; //税额

                            item["APFCMoney_Tax"] = item["APOCMoney_GoodsTax"]; //总税额

                            item["APOCMoney_Tax"] = item["APFCMoney_Tax"]; //总税额



                            string see = item["APOCMoney_TotalMoney"].ToString();
                            string see1 = item["APFCMoney_TotalMoney"].ToString();
                        ...
                        else
                        {
                            ...
                            item["APOCMoneyPriceTaxSum"] = TaxSum;//价税合计

                            item["APOCMoney_NonTax"] = decimal.Parse(item["APOCMoneyPriceTaxSum"].ToString()) - decimal.Parse(item["APOCMoney_GoodsTax"].ToString());//未税金额
