                        ...
                        foreach (var item in NrefSDtos)
                        {
                            ...
                            foreach (IUIRecord record in _part.Model.RefScmBillLineView.Records)
                            {
                                if (record["Item_Code"].ToString() == dataTable.Rows[n]["ItemInfo_ItemCode"].ToString())
                                {
                                    ...
                                    record["PayReqFundHead_DescFlexField_PrivateDescSeg29"] = dataTable.Rows[n]["TotalRecievedQtyCU"].ToString();//入库数量
                                    record["PayReqFundHead_DescFlexField_PrivateDescSeg28"] = dataTable.Rows[n]["TotalConfirmedQtyCU"].ToString();//开票数量
